...
- chunksize: If mode="genome", ProActive will 'chunk' the genome to search for elevations and gaps in read coverage on each chunk. Since ProActive can only identify one region of elevated or gapped read coverage per 'chunk', choosing smaller chunks will increase sensitivity AND processing time. Default is 100,000bp per chunk, with the exception of the last chunk which is the remainder number of base pairs once the genome has been evenly 'chunked'. Chunk size should be divisible by 100.
