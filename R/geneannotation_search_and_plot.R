...
#' @param WC_subset A subset of the read coverage dataset that pertains only to the contig currently being assessed
...
#' @keywords internal
gene_annotation_search_and_plot <- function(subset_gene_annots, keywords, WC_subset, classification, ref_name, colindex, start_bprange, end_bprange){
  ...
  plot <- ggplot(data=WC_subset, aes(x=position, y=coverage))+
    ...
    geom_label(data=gene_annot_subset, aes(x=startpos,y=(max(WC_subset$coverage)/2),label=paste0("#", c(1:nrow(gene_annot_subset)))), size=2.5)+
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          panel.background = element_blank(), axis.line = element_line(colour = "black"),text = element_text(size = 15))+
    labs(title=paste(ref_name,classification), x="Contig position (bp)", caption=gene_annots_labels, y="Read coverage")
  return(plot)
}
