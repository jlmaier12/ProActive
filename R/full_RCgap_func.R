...
#'
#' @keywords internal
full_RCgap_func <- function (microbial_subset, windowsize, minsize, maxsize) {
  ...
  shape <- c(rep(startingcoverages[1], 10000/windowsize),rep(min_read_cov, shape_length), rep(startingcoverages[1], 10000/windowsize))
  ...
  for(cov in startingcoverages) {
    min_read_cov <- min(microbial_subset[,2])
    shape <- c(rep(cov, 10000/windowsize), rep(min_read_cov, shape_length), rep(cov, 10000/windowsize))
    repeat {
      middle_rows <- which(shape == min_read_cov)
      if (length(middle_rows) < minsize/windowsize) break
      best_match_info <- RCgap_translator_func(Cov_values_contig, best_match_info, windowsize, shape)
      if (length(middle_rows)<= 2000/windowsize) break
      shape <- c(shape[-c(middle_rows[2]:middle_rows[(2000/windowsize)+1])], rep(cov,2000/windowsize)) #remove 2000bp at a time
    }
    for(mincov in startingmincoverages) {
      shape <- c(rep(cov, 10000/windowsize), rep(mincov, shape_length), rep(cov, 10000/windowsize))
      repeat {
        middle_rows <- which(shape == mincov)
        if (length(middle_rows) < minsize/windowsize) break
        best_match_info <- RCgap_translator_func(Cov_values_contig, best_match_info, windowsize, shape)
        if (length(middle_rows)<= 2000/windowsize) break
        shape <- c(shape[-c(middle_rows[2]:middle_rows[(2000/windowsize)+1])], rep(cov,2000/windowsize)) #remove 2000bp at a time
